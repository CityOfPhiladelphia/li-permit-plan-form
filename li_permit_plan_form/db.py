...
def get_permit(apno):
    ...
    sql = f"""SELECT * FROM eclipse_plan_app_permit
...
def get_plans(apno):
    db = get_db()
    named_params = {'apno': apno}
    sql = f"""SELECT * FROM eclipse_plan_app_plan
              INNER JOIN eclipse_plan_app_plan_permit ON eclipse_plan_app_plan.id = eclipse_plan_app_plan_permit.plan_id
              INNER JOIN eclipse_plan_app_permit ON eclipse_plan_app_permit.apno = eclipse_plan_app_plan_permit.apno
              WHERE eclipse_plan_app_permit.apno LIKE :apno"""
...
def get_apnos_associated_with_plan(plan_id, apno):
    ...
    sql = f"""SELECT apno
              FROM eclipse_plan_app_permit
              WHERE apno IN 
              (
                  SELECT apno
                  FROM eclipse_plan_app_plan_permit
                  WHERE plan_id = :plan_id
              )
              AND eclipse_plan_app_permit.apno NOT LIKE :apno"""
...
def get_permit_address(apno):
    ...
    db = get_db()
    named_params = {'apno': apno}
    sql = f"""SELECT address FROM eclipse_plan_app_permit
              WHERE apno LIKE :apno"""
...
def insert_plan(package, location, comments):
    ...
    plan_insert_sql = f"""INSERT INTO eclipse_plan_app_plan (
                            package, 
                            location, 
                            comments, 
                            dateadded
...
def insert_plan_permit(apno):
    ...
    plan_permit_insert_sql = f"""INSERT INTO eclipse_plan_app_plan_permit (
                                    plan_id, 
                                    apno
                                 )
                                 VALUES (
...
def delete_plan(plan_id):
    ...
    delete_plan_sql = f"""DELETE FROM eclipse_plan_app_plan
                          WHERE id = :plan_id"""
    db.engine.execute(text(delete_plan_sql), named_params)

    # Delete the plan_permit entries for this plan
    delete_plan_permit_sql = f"""DELETE FROM eclipse_plan_app_plan_permit
                                 WHERE plan_id = :plan_id"""
...
def update_plan(plan_id, package, location, comments):
    ...
    # Insert the plan into the plan table
    plan_insert_sql = f"""UPDATE eclipse_plan_app_plan
                            SET package=:package,
                                location=:location,
                                comments=:comments,
...
def update_plan_permits(plan_id, apnos):
    ...
    plan_permit_delete_sql = f"""DELETE FROM eclipse_plan_app_plan_permit
                                   WHERE plan_id=:plan_id"""
    ...
    for apno in apnos:
        ...
        plan_permit_insert_sql = f"""INSERT INTO eclipse_plan_app_plan_permit (plan_id, apno)
                                    VALUES (:plan_id, :apno)"""
